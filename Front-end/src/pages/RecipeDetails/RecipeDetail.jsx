...
export default function RecipeDetail() {
        ...
        const fetchRecipeDetails = async () => {
            try {
                ...
                if (token) {
                    ...
                    try {
                        const favoriteCheck = await axios.get(`http://localhost:4242/authRecipe/favorite/${id}`, {
                        ...
                        console.warn("Não foi possível verificar o estado do favorito:", favErr);
                    }
                }
    ...
    const toggleFavorite = async () => {
        try {
            if (!token) return;
            await axios.post(`http://localhost:4242/authRecipe/favorite/${id}`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
        ...
        try {
            ...
            if (recipe) {
                ...
                setRecipe({
                    ...
                    _count: { ...recipe._count, reviews: (recipe._count?.reviews || 0) + 1 }
                });
            }
            setComment('');
        } catch (err) {
    ...
    if (loading) return <div className="loading-status">A preparar os detalhes da receita...</div>;
    if (error) return <div className="error-status">{error} <Link to="/recipes">Voltar</Link></div>;
    if (!recipe) return <div className="error-status">Receita não encontrada.</div>;
    ...
    return (
        <div className="recipe-detail-page">
            <div className="return" >
                <Link to="/recipes" className="back-link">← Voltar para as Receitas</Link></div>
            ...
            <div className="recipe-header">
                ...
                <div className="content-title-description">
                    ...
                    <p className="recipe-description-text">
                        {recipe.description || "Esta deliciosa receita não possui uma descrição detalhada, mas o seu modo de preparo fala por si!"}
                    </p>
                    <div>
                        <p className="recipe-favorites-count">❤️ {totalFavorites} pessoas adicionaram aos favoritos</p>
                    {isUserLoggedIn ? (
                        <button
                    ...
                    {recipe.ingredients && recipe.ingredients.length > 0 ? (
                        <ul className="ingredients-list">
                            {recipe.ingredients.map((item, index) => (
                                <li key={index}>
                                    <span className="ingredient-qnt">{item.qnt}{item.measure?.abbreviation}</span>
                ...
                {isUserLoggedIn ? (
                    <form onSubmit={handleReviewSubmit} className="review-form">
                        ...
                        <p className="rating-text">
                            {userRating > 0 ? `A tua nota: ${userRating} de 5 estrelas` : "Seleciona a estrela acima!"}
                        </p>
                    ...
                    <p className="login-alert">
                        💡 Gostaste do prato? <Link id="link" to="/login"><strong>Faz login</strong></Link> para deixar a tua avaliação e comentário!
                    </p>
                )}
            </div>
            ...
            <div className="reviews-section">
                ...
                {recipe.reviews && recipe.reviews.length > 0 ? (
                        ...
                        {recipe.reviews.map((review, index) => (
                            <div key={index} className="review-card-item">
                                <div className="review-card-header">
                                    <strong className="review-user-name">{review.user?.name}</strong>
                    ...
                    <p className="no-reviews">Ainda ninguém comentou. Seja o primeiro a cozinhar e a deixar um feedback!</p>
                )}
            </div>
        </div>
    );
